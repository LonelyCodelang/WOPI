...
namespace MainWeb.Helpers
{
    public class WopiAppHelper
    {
        ...
        public string  GetDocumentLink(string wopiHostandFile,string linkType="view")
        {
            ...
            var rv = GetDocumentLink(appName.name, fileExt, wopiHostandFile, accessToken, linkType);
        ...
        const string s_WopiHostFormat = "WOPISrc={0}&access_token={1}";
        public string GetDocumentLink(string appName, string fileExtension, string wopiHostAndFile, string accessToken,string linkType="view")
        {
            ...
            var appAction = appStuff.action.Where(c => c.ext == fileExtension && c.name==linkType).FirstOrDefault();
                        if (null == appAction)
                throw new ApplicationException("Can't locate UrlSrc for : " + appName);

            //var endPoint = appAction.urlsrc.IndexOf('?');
            var endPoint = appAction.urlsrc.Replace("<ui=UI_LLCC&>", "");
            endPoint = endPoint.Replace("<rs=DC_LLCC&>", "");
            endPoint = endPoint.Replace("<showpagestats=PERFSTATS&>", "");
            var fullPath = string.Format(endPoint+ s_WopiHostFormat, wopiHostUrlsafe, accessToken); 
